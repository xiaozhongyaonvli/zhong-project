...
int main(int argc, char** argv){
    ...
    /*高斯模糊去噪*/
    cv::Mat blurred;
    cv::GaussianBlur(, blurred, cv::Size(5,5), 0); //这里的最后一个0 sigmaX：高斯核在 X 方向上的标准差。参数为0表示函数会根据高斯核的大小自动计算标准差。
    ...
    /*轮廓检测*/
    std::vector<std::vector<cv::Point> > contours; //一个参数是点的合集(一个轮廓)的容器
    ...
    cv::drawContours(outline, contours, -1, cv::Scalar(0,0,255),2); //-1表示化所有，也可以指定某个，Scalar表示红色线，下一个参数表示2厚度
