...
bool compare_nums(const std::vector<cv::Point>& a, const std::vector<cv::Point>& b) 
{
    ...
    return bounding_rect1.x<bounding_rect2.x; //根据他们轮廓矩形的列来排序
}
...
int main(int argc, char** argv){
    ...
    cv::Mat templ_gray;
    cv::cvtColor(templ. templ_gray, cv::COLOR_B)
    ...
    sort(contours.begin(),contours.end(), compare_nums); //这里对轮廓进行排序
    ...
    {   //这里化轮廓矩形
        ...
        std::cout<<"bounding_rect  "<<i<<".x=="<<bounding_rect.x<<std::endl;
        std::cout<<"bounding_rect  "<<i<<".w=="<<bounding_rect.width<<std::endl;
        cv::rectangle(outline, bounding_rect, cv::Scalar(0, 0, 255), 3);
        cv::Mat roi = outline(bounding_rect);
